...
<section class="py-16 bg-gray-50">
  <div class="container mx-auto px-12">
    ...
    <?php
    // First, get the latest 3 posts for the grid
    ...
    // Get the IDs of the posts already in the grid to exclude them from featured
    $exclude_ids = array();
    if ($latest_posts->have_posts()) {
        ...
        $latest_posts->rewind_posts(); // Reset the loop so we can use it again
    }

    // Query for ONE featured post, excluding the ones already in the grid
    $main_featured = new WP_Query(array(
        'posts_per_page' => 1,
        'post_status' => 'publish',
        'post__not_in' => $exclude_ids, // Exclude posts already in grid
        'ignore_sticky_posts' => true
    ));

    // If you're using ACF for featured posts, use this instead:
    /*
    $main_featured = new WP_Query(array(
        'posts_per_page' => 1,
        'post_status' => 'publish',
        ...
        'meta_query' => array(
            array(
                'key' => 'featured_article', // Your ACF field name
                'value' => '1',
                'compare' => '='
            )
        )
